...
for task in "${tasks[@]}"; do
    python test.py --data_dir GUE/${task} \
                --model_name_or_path /scratch/anonymous/Attack_Benchmark/models/ADFAR/GUE/GUE/${model}/${task}/4times_adv_double_0-7 \
                --task_name 0 --num_label 2 --n_gpu 1 \
                --max_seq_length 256 --batch_size 16 \
                --output_dir PGD/results/ADFAR/${model}/${task} \
