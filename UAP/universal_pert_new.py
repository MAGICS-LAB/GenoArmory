...
def universal_perturbation_nlp(
    dataset, valset, model, tokenizer, delta=0.7, max_iter_uni=10,
    ...
    while fooling_rate < 1 - delta and iteration < max_iter_uni:
        print(f"[{iteration}] Current fooling rate: {fooling_rate:.6f}")
        ...
        for batch in dataloader:
            ...
            if v is None:
                v = torch.zeros_like(embeddings).to(device)
        ...
        print(f"[{iteration}] Updated fooling rate: {fooling_rate:.6f}, ||v||={v.norm().item():.4f}")

        # Early stopping check
        if abs(fooling_rate - last_fooling) < 1e-5:
            patience_counter += 1
        else:
            patience_counter = 0
        last_fooling = fooling_rate

        if patience_counter >= patience:
            print("Early stopping triggered: fooling rate did not improve for 3 iterations.")
            break

        iteration += 1

    print(f"Final fooling rate: {fooling_rate:.6f}")
    return v
