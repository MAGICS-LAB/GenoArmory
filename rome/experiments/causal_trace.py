...
def main():
    ...
    aa("--gene_file",default='/projects/p32013/DNA/data/GUE/prom/prom_300_all/test.csv')
    ...
    if args.fact_file is None and args.gene_file is None:
        ...
        knowns = pd.read_csv(args.gene_file)
    else:
    ...
    for known_id in tqdm(range(len(knowns))):
        knowledge = knowns.iloc[known_id]
        for kind in "all", "mlp", "attn":
            ...
            if not numpy_result["correct_prediction"]:
                tqdm.write(f"Skipping {knowledge['sequence']}")
                continue
            plot_result = dict(numpy_result)
            plot_result["kind"] = kind
            pdfname = f'{pdf_dir}/{str(numpy_result["answer"]).strip()}_{known_id}{kind_suffix}.pdf'

            plot_trace_heatmap(plot_result, savepdf=pdfname, modelname=modelname)
