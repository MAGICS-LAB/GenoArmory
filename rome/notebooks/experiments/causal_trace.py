...
from palettable.lightbartlein.diverging import BlueOrange10_9
...
def main():
    ...
    aa("--gene_dict",default='/projects/p32013/DNABERT-meta/rome/notebooks/dna_data')
    aa("--gene_file",default='prom_300_all_test.json')
    aa("--output_dir", default="results/{model_name}/causal_trace1")
    ...
    if args.model_name == 'geneocean':
        args.model_name = '/projects/p32013/DNABERT-meta/meta-100M'
    ...
    print(args.gene_file)
    if args.fact_file is None and args.gene_file is None:
        knowns = KnownsDataset(DATA_DIR)
    elif args.fact_file is None:
        knowns = DNADataset(args.gene_dict, args.gene_file )
    else:
    ...
    for knowledge in tqdm(knowns):
        ...
        for kind in "all", "mlp", "attn":
            ...
            plot_trace_heatmap(plot_result, savepdf=pdfname, modelname='geneocean')
...
def guess_subject(prompt, length=10):
    """
    Randomly selects a subsequence of the given length from the DNA sequence.

    Args:
        prompt (str): The DNA sequence.
        length (int): Length of the subsequence to extract (default is 10).

    Returns:
        str: A randomly selected subsequence.
    """

    if len(prompt) < length:
        raise ValueError("The DNA sequence is shorter than the requested subject length.")
    
    start_index = random.randint(0, len(prompt) - length)
    subject = prompt[start_index : start_index + length]
    print(subject)
    return subject
...
def plot_trace_heatmap(result, savepdf=None, title=None, xlabel=None, modelname=None):
    ...
    for i in range(*result["subject_range"]):
        labels[i] = labels[i] + "*"
    oclor = BlueOrange10_9.mpl_colors
    with plt.rc_context(rc={}):
        fig, ax = plt.subplots(figsize=(3.5, 2), dpi=200)
        h = ax.pcolor(
            differences,
            cmap={None: "Purples", "all": "Greens", "mlp": "Oranges", "attn": "Purples", "gate":"Blues"}[
                kind
            ],
            vmin=0.1,
            vmax=0.5,
        ...
        if not kind:
            ...
            kind_dict = {'mlp': "MLP", 'attn':'Attn', 'all':'All'}
            kindname = kind_dict[kind]
...
def predict_token(mt, prompts, return_p=False):
    # Add [CLS] token manually to match generate_sequences input formatting
    prompts = ["[CLS]" + p for p in prompts]
