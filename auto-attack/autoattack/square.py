...
class SquareAttack():
    ...
    def margin_and_loss(self, x, y):
        ...
        logits = self.predict(inputs_embeds=x, attention_mask=attention_mask, token_type_ids=token_type_ids).logits
    ...
    def perturb(self, x, y=None, attention_mask=None, token_type_ids=None):
        """
        ...
        if y is None:
            if not self.targeted:
                with torch.no_grad():
                    output = self.predict(inputs_embeds=x, attention_mask=attention_mask, token_type_ids=token_type_ids).logits
        ...
        if not self.targeted:
            acc = self.predict(inputs_embeds=x, attention_mask=attention_mask, token_type_ids=token_type_ids).logits.max(1)[1] == y
        else:
            acc = self.predict(inputs_embeds=x, attention_mask=attention_mask, token_type_ids=token_type_ids).logits.max(1)[1] != y
        ...
        for counter in range(self.n_restarts):
            ...
            if ind_to_fool.numel() != 0:
                ...
                output_curr = self.predict(inputs_embeds=adv_curr, attention_mask=attention_mask, token_type_ids=token_type_ids).logits
                if not self.targeted:
