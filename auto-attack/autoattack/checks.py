...
def check_randomized(model, x, y, bs=250, n=5, alpha=1e-4, logger=None, attention_mask=None, token_type_ids=None):
    acc = []
    corrcl = []
    outputs = []
    with torch.no_grad():
        for _ in range(n):
            output = model(x, attention_mask=attention_mask, token_type_ids=token_type_ids)
            if hasattr(output, "logits"):
                logits = output.logits
            else:
                logits = output  # fallback if already tensor
            y = y.to(logits.device)
            corrcl_curr = (logits.max(1)[1] == y).sum().item()
            corrcl.append(corrcl_curr)
            outputs.append(logits / (L2_norm(logits, keepdim=True) + 1e-10))
...
def check_range_output(model, x, alpha=1e-5, logger=None, attention_mask=None, token_type_ids=None):
    with torch.no_grad():
        output = model(x, attention_mask=attention_mask, token_type_ids=token_type_ids)
    if hasattr(output, "logits"):
        logits = output.logits
        ...
        logits = output  # fallback if already tensor
    fl = [logits.max() < 1. + alpha, logits.min() >  -alpha,
        ((logits.sum(-1) - 1.).abs() < alpha).all()]
    if all(fl):
...
def check_dynamic(model, x, is_tf_model=False, logger=None, attention_mask=None, token_type_ids=None):
    if is_tf_model:
        ...
        print(model)
        model(inputs_embeds=x, attention_mask=attention_mask, token_type_ids=token_type_ids)
        sys.settrace(None)
...
def check_n_classes(n_cls, attacks_to_run, apgd_targets, fab_targets,
    logger=None, attention_mask=None, token_type_ids=None):
    msg = None
