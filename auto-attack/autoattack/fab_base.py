...
class FABAttack():
    ...
    def perturb(self, x, y, attention_mask=None, token_type_ids=None):
        if self.device is None:
            self.device = x.device
        adv = x.clone()
        with torch.no_grad():
            acc = self._predict_fn(x, attention_mask=attention_mask, token_type_ids=token_type_ids).max(1)[1] == y
            ...
            if not self.targeted:
                for counter in range(self.n_restarts):
                    ...
                    if ind_to_fool.numel() != 0:
                        ...
                        acc_curr = self._predict_fn(adv_curr, attention_mask=attention_mask, token_type_ids=token_type_ids).max(1)[1] == y_to_fool
                        if self.norm == 'Linf':
