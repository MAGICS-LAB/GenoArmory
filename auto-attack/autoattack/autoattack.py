...
class AutoAttack():
    ...
    def get_logits(self, x, attention_mask=None, token_type_ids=None):
        if not self.is_tf_model:
            output = self.model(inputs_embeds=x, attention_mask=attention_mask, token_type_ids=token_type_ids)
    ...
    def run_standard_evaluation(self,
        ...
        if self.version != 'rand':
            checks.check_randomized(self.get_logits, x_orig[:bs].to(self.device),
                y_orig[:bs].to(self.device), bs=bs, logger=self.logger, attention_mask=attention_mask, token_type_ids=token_type_ids)
        ...
        checks.check_dynamic(self.model, x_orig[:bs].to(self.device), self.is_tf_model,
            logger=self.logger, attention_mask=attention_mask, token_type_ids=token_type_ids)
        checks.check_n_classes(n_cls, self.attacks_to_run, self.apgd_targeted.n_target_classes,
            self.fab.n_target_classes, logger=self.logger, attention_mask=attention_mask, token_type_ids=token_type_ids)
        ...
        with torch.no_grad():
            ...
            if state.robust_flags is None:
                ...
                for batch_idx in range(n_batches):
                    ...
                    # Extract attention_mask and token_type_ids for this batch
                    attn_mask = attention_mask[start_idx:end_idx, :].clone().to(self.device) if attention_mask is not None else None
                    token_type = token_type_ids[start_idx:end_idx, :].clone().to(self.device) if token_type_ids is not None else None
                    
                    output = self.get_logits(x, attention_mask=attn_mask, token_type_ids=token_type).max(dim=1)[1]
            ...
            for attack in attacks_to_run:
                ...
                for batch_idx in range(n_batches):
                    ...
                    # NEW: extract attention_mask and token_type_ids for this batch
                    attn_mask = attention_mask[batch_datapoint_idcs, :].clone().to(self.device) if attention_mask is not None else None
                    token_type = token_type_ids[batch_datapoint_idcs, :].clone().to(self.device) if token_type_ids is not None else None

                    # For NLP embeddings, we don't need to add extra dimensions
                    # The embeddings should remain 3D: [batch_size, sequence_length, hidden_size]
                    # Only add dimension if it's a single sample and we need to maintain batch dimension
                    if len(x.shape) == 2:  # Single sample: [sequence_length, hidden_size]
                        x.unsqueeze_(dim=0)  # Make it [1, sequence_length, hidden_size]
                        if attn_mask is not None and len(attn_mask.shape) == 1:
                            attn_mask.unsqueeze_(dim=0)
                        if token_type is not None and len(token_type.shape) == 1:
                            token_type.unsqueeze_(dim=0)
                    ...
                    if attack == 'apgd-ce':
                        ...
                        adv_curr = self.apgd.perturb(x, y, attention_mask=attn_mask, token_type_ids=token_type) #cheap=True
                        ...
                        adv_curr = self.fab.perturb(x, y, attention_mask=attn_mask, token_type_ids=token_type)
                    
                    elif attack == 'square':
                        # square
                        self.square.seed = self.get_seed()
                        adv_curr = self.square.perturb(x, y, attention_mask=attn_mask, token_type_ids=token_type)
                    
                    elif attack == 'apgd-t':
                        # targeted apgd
                        self.apgd_targeted.seed = self.get_seed()
                        adv_curr = self.apgd_targeted.perturb(x, y, attention_mask=attn_mask, token_type_ids=token_type) #cheap=True
                    ...
                    output = self.get_logits(adv_curr, attention_mask=attn_mask, token_type_ids=token_type).max(dim=1)[1]
    ...
    def clean_accuracy(self, x_orig, y_orig, bs=250, attention_mask=None, token_type_ids=None):
        ...
        for counter in range(n_batches):
            ...
            # Extract attention_mask and token_type_ids for this batch
            attn_mask = attention_mask[counter * bs:min((counter + 1) * bs, x_orig.shape[0])].clone().to(self.device) if attention_mask is not None else None
            token_type = token_type_ids[counter * bs:min((counter + 1) * bs, x_orig.shape[0])].clone().to(self.device) if token_type_ids is not None else None
            
            output = self.get_logits(x, attention_mask=attn_mask, token_type_ids=token_type)
    ...
    def run_standard_evaluation_individual(self, x_orig, y_orig, bs=250, return_labels=False, attention_mask=None, token_type_ids=None):
        if self.verbose:
        ...
        for c in l_attacks:
            ...
            x_adv, y_adv = self.run_standard_evaluation(x_orig, y_orig, bs=bs, return_labels=True, attention_mask=attention_mask, token_type_ids=token_type_ids)
            if return_labels:
                adv[c] = (x_adv, y_adv)
            else:
                adv[c] = x_adv
            if verbose_indiv:    
                acc_indiv  = self.clean_accuracy(x_adv, y_orig, bs=bs, attention_mask=attention_mask, token_type_ids=token_type_ids)
                space = '\t \t' if c == 'fab' else '\t'
    ...
    def set_version(self, version='standard'):
        ...
        if version == 'standard':
            self.attacks_to_run = ['apgd-ce', 'fab-t', 'square', 'apgd-t']
